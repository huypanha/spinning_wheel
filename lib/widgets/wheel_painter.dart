...
class WheelPainter extends CustomPainter {
  ...
  WheelPainter(this.segments, {this.imageHeight, this.imageWidth, this.labelStyle, this.slicePadding = EdgeInsets.zero});
  ...
  void _drawImage(Canvas canvas, double radius, double angle, double segmentAngle, WheelSegment segment) {
    if (segment.image != null) {
      ...
      final double imageRadius = (radius * 0.55) - slicePadding.top + slicePadding.bottom;
  ...
  void _drawLabel(Canvas canvas, double radius, double angle, double segmentAngle, WheelSegment segment) {
    ...
    final double labelRadius = (radius * 0.75) - (slicePadding.top + slicePadding.bottom);
    ...
    canvas.rotate(angle + segmentAngle / 2 + pi / 2 + (labelStyle?.angle ?? 0.0));

    final TextStyle? effectiveStyle = labelStyle?.labelStyle;

    // Calculate available width at this radius minus horizontal padding
    final double baseWidth = 2 * labelRadius * sin(segmentAngle * 0.7);
    final double availableWidth = baseWidth;

    TextPainter textPainter = TextPainter(
      text: TextSpan(
        text: segment.label,
        style: effectiveStyle ?? const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold),
      ),
    ...
    textPainter.paint(canvas, Offset(-textPainter.width * .7, -textPainter.height / 2));
    canvas.restore();
  }

  @override
  bool shouldRepaint(WheelPainter oldDelegate) => oldDelegate.segments != segments || oldDelegate.labelStyle != labelStyle || oldDelegate.slicePadding != slicePadding;
}
